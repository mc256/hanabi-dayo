...
export async function updateOverrideItem(item: OverrideItem): Promise<void> {
  ...
  if (index === -1) {
    throw new Error(t('override.notFound'))
  }
...
export async function createOverride(item: Partial<OverrideItem>): Promise<OverrideItem> {
  ...
  const newItem = {
    id,
    name: item.name || (item.type === 'remote' ? t('override.remoteFile') : t('override.localFile')),
    type: item.type,
    ext: item.ext || 'js',
    url: item.url,
  ...
  switch (newItem.type) {
    case 'remote': {
      ...
      if (!item.url) throw new Error(t('override.emptyUrl'))
